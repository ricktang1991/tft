...
let API_KEY = valueForAPIKey(named: "API_key")
...
struct MatchHistoryRequest: Decodable {
    ...
    init(summonerPuuid: String) {
        let resourceString = "https://americas.api.riotgames.com/tft/match/v1/matches/by-puuid/\(summonerPuuid)/ids?count=20&api_key=\(API_KEY)"
        guard let resourceURL = URL(string: resourceString) else { fatalError() }
